...
function helios(p) {
  var T=(astromath.getJulianDate()-2451545.0)/36525;
  var T2=T*T;
  var T3=T2*T;
...
function radecr(obj,base) {
  // Equatorial co-ordinates
  var x=obj[0];
  var y=obj[1];
  var z=obj[2];
  // julian date
  var jdobs=astromath.getJulianDate();
...
//console.log(radecr(helios(planets[4]),helios(planets[2]))[0],radecr(helios(planets[4]),helios(planets[2]))[1]);
